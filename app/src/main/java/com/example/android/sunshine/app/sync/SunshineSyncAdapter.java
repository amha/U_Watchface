...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements
    ...
    // data items to send to the watch face
    double high;
    double low;
    String cityName;
    int weatherId;
    ...
    public void onConnected(Bundle bundle) {
        Log.d("AM-Out", "Connected to device");
        Wearable.DataApi.addListener(mGoogleApiClient, this);
    }

    @Override
    public void onConnectionSuspended(int i) {
        Log.d("AM-Out", "Connected to suspended");
    }

    @Override
    public void onConnectionFailed(ConnectionResult connectionResult) {
        Log.d("AM-Out", "Connection Failed");
        ...
        try {
            ...
            if (cVVector.size() > 0) {
                ...
                if(mGoogleApiClient.isConnected()) {
                    ...
                    Log.d("DEVICE", "Google client connected");
                    Log.d("DEVICE", "City: " + cityName + " High: " + high + " Low: " + low);

                    PutDataMapRequest dataMap = PutDataMapRequest.create("/weather");

                    dataMap.getDataMap().putString("city", cityName);
                    dataMap.getDataMap().putString("high_temp", ""+high);
                    dataMap.getDataMap().putString("low_temp", ""+low);
                    dataMap.getDataMap().putInt("id", weatherId);
                    PutDataRequest putDataReq = dataMap.asPutDataRequest();
...
//                            Log.d("AMHA Result", "Results : " + result.getStatus());
//                        }
//                    });
                }
                else {
                    Log.d("DEVICE", "Google client not connected");
                }
